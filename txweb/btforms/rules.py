...
is_alphanum = lambda x: btforms.regexp("^[A-Za-z0-9]{%s}$" % x, u"必须是长度为%s的数字或字母" % x)
is_alphanum2 = lambda x, y: btforms.regexp("^[A-Za-z0-9]{%s,%s}$" % (x, y), u"必须是长度为%s到%s的数字或字母" % (x, y))
is_number = btforms.regexp("^[0-9]*$", u"必须是数字")
is_number2 = btforms.regexp("^[1-9]\d*$",u'必须是大于0的正整数')
is_number3 = btforms.regexp('^(([0-9]\d*)|0)(\.\d{1,3})?$', u"支持包含(最大3位)小数点 xx.xxxxx")
